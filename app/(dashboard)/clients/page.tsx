...
export default function ClientsPage() {
  ...
  const loadClientDetails = async (client: Client) => {
    ...
    const { data: assignData } = await supabase
      ...
      .select('*, users:team_member_user_id(full_name, email)')
      .eq('client_id', client.id)
    ...
    if (!error && newClient) {
      for (const assign of form.assigned_users) {
        await supabase.from('team_assignments').insert({
          ...
          team_member_user_id: assign.user_id,
          role_on_account: assign.role,
        })
      }
    }
  ...
  const assignToClient = async () => {
    ...
    await supabase.from('team_assignments').insert({
      ...
      team_member_user_id: assignUserId,
                      ...
                      <td className="px-5 py-4">
                        <div className="flex -space-x-2">
                          {clientAssigns.slice(0, 3).map((a) => (
                            <div key={a.id} className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white border-2 border-white" style={{ backgroundColor: getColor(a.team_member_user_id) }} title={a.users?.full_name || a.role_on_account}>
                              {getInitials(a.users?.full_name || a.users?.email || '?')}
                            </div>
                          ))}
                          {clientAssigns.length > 3 && (
                            <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-medium border-2 border-white" style={{ backgroundColor: '#EBE3D3', color: 'rgba(42,37,32,0.5)' }}>+{clientAssigns.length - 3}</div>
                          )}
              ...
              <div className="flex items-center gap-1">
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                  <button key={p} onClick={() => setPage(p)} className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-medium" style={{
                    backgroundColor: page === p ? '#2A2520' : 'transparent', color: page === p ? '#FFFFFF' : 'rgba(42,37,32,0.5)', border: page === p ? 'none' : '1px solid #EBE3D3',
                  }}>{p}</button>
                ))}
        ...
        <div className="w-96 ml-6 bg-white rounded-xl border border-cream flex flex-col overflow-hidden" style={{ height: 'calc(100vh - 130px)' }}>
          ...
          <div className="flex border-b border-cream">
            {(['overview', 'team', 'files'] as const).map(tab => (
              <button key={tab} onClick={() => setDetailTab(tab)} className="flex-1 py-3 text-xs font-medium capitalize" style={{
              ...
              <div>
                <div className="space-y-3 mb-6">
                  {clientAssignments.length === 0 && (
                    <p className="text-sm py-4 text-center" style={{ color: 'rgba(42,37,32,0.3)' }}>No team members assigned.</p>
                  )}
                  {clientAssignments.map(a => (
                    <div key={a.id} className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: '#FAF8F0' }}>
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white" style={{ backgroundColor: getColor(a.team_member_user_id) }}>
                          {getInitials(a.users?.full_name || a.users?.email || '?')}
                        </div>
                        <div>
                ...
                <div className="border-t border-cream pt-4">
                  ...
                  <select value={assignUserId} onChange={(e) => setAssignUserId(e.target.value)} className="w-full px-3 py-2 border border-cream rounded-lg text-sm bg-white mb-2">
                    ...
                    {teamUsers.filter(u => !clientAssignments.some(a => a.team_member_user_id === u.id)).map(u => (
            ...
            {detailTab === 'files' && (
              <div>
                ...
                <div
                  ...
                  className="border-2 border-dashed rounded-xl p-6 text-center cursor-pointer mb-4 transition-colors hover:border-amber-300"
                  style={{ borderColor: '#EBE3D3', backgroundColor: '#FFFDF0' }}
                  tabIndex={0}
                >
                  <p className="text-sm font-medium mb-1" style={{ color: '#2A2520' }}>
                    {uploading ? 'Uploading...' : 'Drop files, paste an image, or click to upload'}
                  </p>
                  <p className="text-xs" style={{ color: 'rgba(42,37,32,0.4)' }}>Supports all file types and pasted screenshots</p>
                ...
                <div className="space-y-2">
                  {clientFiles.length === 0 && (
                    <p className="text-sm py-4 text-center" style={{ color: 'rgba(42,37,32,0.3)' }}>No files yet.</p>
                  )}
                    ...
                    return (
                      <div key={file.id} className="flex items-center gap-3 p-3 rounded-lg border border-cream/50" style={{ backgroundColor: '#FAF8F0' }}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs" style={{ color: 'rgba(42,37,32,0.4)' }}>{formatSize(file.file_size)} · {file.users?.full_name || ''}</p>
                        </div>
                        <div className="flex gap-1 flex-shrink-0">
                          ...
                          <button onClick={() => deleteFile(file.id, file.file_path)} className="text-xs px-2 py-1 rounded border border-cream hover:bg-red-50" style={{ color: '#ef4444' }}>x</button>
                        </div>
                      </div>
                    )
                  })}
              ...
              <div>
                  ...
                  return (
                    <div key={a.user_id} className="flex items-center justify-between p-2 rounded-lg mb-1" style={{ backgroundColor: '#FAF8F0' }}>
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white" style={{ backgroundColor: getColor(a.user_id) }}>
                ...
                <div className="flex gap-2">
                  <select value={assignUserId} onChange={(e) => setAssignUserId(e.target.value)} className="flex-1 px-3 py-2 border border-cream rounded-lg text-sm bg-white">
                    <option value="">Select member</option>
                    {teamUsers.filter(u => !form.assigned_users.some(a => a.user_id === u.id)).map(u => (
                      <option key={u.id} value={u.id}>{u.full_name || u.email}</option>
                    ))}
                  </select>
