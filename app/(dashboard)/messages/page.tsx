  ...
  return (
    <div className="flex rounded-xl border border-cream overflow-hidden bg-white" style={{ height: 'calc(100vh - 130px)' }}>
      ...
      <div className="w-64 border-r border-cream flex flex-col" style={{ backgroundColor: '#FAF8F0' }}>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-3 pt-4 pb-2">
            ...
            {channels.filter(c => c.type !== 'dm').map((ch) => (
              <button
                key={ch.id}
                onClick={() => { setActiveChannel(ch); setShowDetails(false) }}
          ...
          <div className="px-3 pt-2 pb-4">
            ...
            {users.filter(u => u.id !== userId).slice(0, 5).map((u) => (
              <div key={u.id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm cursor-pointer hover:bg-cream/50 transition-colors">
        ...
        <div className="px-3 py-3 border-t border-cream">
          <div className="rounded-lg p-3" style={{ backgroundColor: '#FFFDF0' }}>
            ...
            <p className="text-xs" style={{ color: 'rgba(42,37,32,0.5)' }}>Use channels to keep team conversations organized.</p>
          </div>
        </div>
        ...
        {activeChannel ? (
          <>
            ...
            <div className="px-6 py-3 border-b border-cream flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-base font-semibold" style={{ color: '#2A2520' }}># {activeChannel.name}</h3>
                </div>
                <p className="text-xs" style={{ color: 'rgba(42,37,32,0.4)' }}>Discussion about {activeChannel.name}</p>
              </div>
              <div className="flex items-center gap-3">
                <div className="flex -space-x-2">
                  ...
                  {users.length > 4 && (
                    <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-medium border-2 border-white" style={{ backgroundColor: '#EBE3D3', color: 'rgba(42,37,32,0.5)' }}>
                      +{users.length - 4}
                    </div>
                  )}
                </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {messages.length === 0 && (
                <div className="text-center py-16">
                  ...
                  <h3 className="text-lg font-semibold mb-1" style={{ color: '#2A2520' }}>Welcome to #{activeChannel.name}</h3>
                  <p className="text-sm" style={{ color: 'rgba(42,37,32,0.4)' }}>This is the start of the conversation. Send a message to get things going.</p>
                </div>
              )}
            ...
            <div className="px-6 py-4 border-t border-cream">
              <form onSubmit={sendMessage}>
                <div className="border border-cream rounded-xl overflow-hidden" style={{ backgroundColor: '#FFFFFF' }}>
                  <input
                    type="text"
                    value={newMessage}
                  ...
                  <div className="flex items-center justify-between px-3 py-2 border-t border-cream/50">
                    <div className="flex items-center gap-1">
                      <button type="button" className="w-8 h-8 rounded flex items-center justify-center hover:bg-cream/50" style={{ color: 'rgba(42,37,32,0.35)' }}>
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-sm mb-4" style={{ color: 'rgba(42,37,32,0.4)' }}>Create a channel to start messaging your team.</p>
      ...
      {showDetails && activeChannel && (
        <div className="w-72 border-l border-cream flex flex-col overflow-y-auto" style={{ backgroundColor: '#FFFFFF' }}>
          <div className="px-5 py-4 border-b border-cream flex items-center justify-between">
            <h3 className="text-sm font-semibold" style={{ color: '#2A2520' }}>Channel Details</h3>
          ...
          <div className="px-5 py-5 border-b border-cream text-center">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-3" style={{ backgroundColor: '#FAF8F0' }}>
              <span className="text-xl font-bold" style={{ color: 'rgba(42,37,32,0.4)' }}>#</span>
            </div>
            <h4 className="text-base font-semibold" style={{ color: '#2A2520' }}># {activeChannel.name}</h4>
          ...
          <div className="px-5 py-4 border-b border-cream">
            ...
            <p className="text-sm" style={{ color: 'rgba(42,37,32,0.6)' }}>All discussions related to {activeChannel.name}.</p>
          </div>
          ...
          <div className="px-5 py-4 border-b border-cream">
            ...
            <div className="flex -space-x-2">
              ...
              {users.length > 6 && (
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium border-2 border-white" style={{ backgroundColor: '#EBE3D3', color: 'rgba(42,37,32,0.5)' }}>
                  +{users.length - 6}
                </div>
              )}
            </div>
          ...
          <div className="px-5 py-4">
            <div className="flex items-center justify-between mb-3">
              <h5 className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'rgba(42,37,32,0.4)' }}>Notifications</h5>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm" style={{ color: 'rgba(42,37,32,0.6)' }}>All new messages</span>
